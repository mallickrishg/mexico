function [K1111,K1211,K1311,K2211,K2311,K3311, ...
          ...
          K1133,K1233,K1333,K2233,K2333,K3333]=computeStressKernelsTetrahedronShearZone(shz,rcv,G,nu,varargin)
% computeStressKernelsTetrahedronShearZone computes the stress on a shear zone due to
% strain confined in a tetrahedral shear zone.
%
...
%    K3311,K3312,K3313,K3322,K3323,K3333]=COMPUTESTRESSKERNELSTETRAHEDRONSHEARZONE(shz,rcv,G,nu)
%
% where nu is Poisson's ratio, G is the shear modulus. shz is a structure
% with shear zone geometry:
%
%   shz.x         : position of vertices
%   shz.vertices  : the four vertices of the tetrahedra
%
...
unicycle.utils.textprogressbar('# tetrahedral shear zone stress kernels: ');
...
for j=1:6
    ...
    for k=1:N
        ...
        [s22,s12,s23,s11,s13,s33]=unicycle.greens.computeStressTetrahedronShearZoneMixedQuad( ...
            rcv.xc(:,2),rcv.xc(:,1),-rcv.xc(:,3), ...
            matlab2relax*shz.x(shz.vertices(k,1),:)', ...
            matlab2relax*shz.x(shz.vertices(k,2),:)', ...
            matlab2relax*shz.x(shz.vertices(k,3),:)', ...
            matlab2relax*shz.x(shz.vertices(k,4),:)', ...
            e(1,j),e(2,j),e(3,j),e(4,j),e(5,j),e(6,j), ...
            G,nu, ...
            'radius',optionStruct.radius);
        s13=-s13;
        s23=-s23;
